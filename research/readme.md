...
- [`friction-pattern-synthesis/`](./friction-pattern-synthesis/) — Cross-task friction taxonomy: aggregates 53 `friction-log.md` files (33 task + 20 research) into FL distribution histogram, 8-category root-cause taxonomy with frequency counts, per-spec attribution table for 18 FL2+ entries, and 4 verbatim TASK.md / FRUSTRATED.md amendment proposals with file:line targets. Output: [`output/SPEC.md`](./friction-pattern-synthesis/output/SPEC.md). Closes [Task 033](../tasks/033-task-spec-integration/task.md) ST-1.
...
- [`spec-staleness-decision-formalization/`](./spec-staleness-decision-formalization/) — Deterministic decision tree (3 levels, 4 leaves, ≤16 lines of pseudocode) mapping observable repo signals → the four `MAINTENANCE.md §3.4` staleness buckets. Output: [`output/SPEC.md`](./spec-staleness-decision-formalization/output/SPEC.md). Consumed by `tools/fm/check-task-lifecycle-classification.py` (Task 033 ST-4 / Task 039 ST-4).
...
- [`gemini/deepwiki-rendering-conventions-agentic-workflows/result.md`](./gemini/deepwiki-rendering-conventions-agentic-workflows/result.md) — Gemini global survey of AI-driven repository intelligence: DeepWiki rendering architecture, `.devin/wiki.json` and `llms.txt`/`AGENTS.md` conventions, agentic workflow economics (ACUs, Ask Devin), ProdE vs. DeepWiki benchmark, open-source alternatives (`deepwiki-open`, `deepwiki-rs`), MCP integration, and compliance implications. Downstream analysis: [`tasks/051-deepwiki-rendering-conventions-agentic-workflows/`](../tasks/051-deepwiki-rendering-conventions-agentic-workflows/).
