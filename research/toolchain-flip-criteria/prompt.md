...
1. The agent MUST treat the following preamble as authoritative orientation before executing any subsequent step: Run research-prompt-optimizer Phase 1–3. Skip Phase 1 askuser; intent canonical. Render to /research/toolchain-flip-criteria/research-prompt.md. Execute and produce /research/toolchain-flip-criteria/output/SPEC.md. Author reflection/friction-log.md. Run tools/check-governance.sh. Commit "research(toolchain-flip-criteria): mechanical flip checklist (Task 039 ST-1)". Do NOT push.
...
4. The agent SHOULD author or update `tasks/039-maintenance-spec-integration/friction-log.md` per FRUSTRATED.md FL[0-3] when frictions arise; absence of frictions MAY be recorded as `FL: 0`.
5. The agent MUST commit with a message that names `Task 039 ST-1` in its trailer; the agent MUST NOT push (the maintainer pushes after review).
