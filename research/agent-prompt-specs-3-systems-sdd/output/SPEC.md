...
- Rationales, descriptions, motivation paragraphs, and prose explanations **MUST NOT** contain RFC 2119 keywords in uppercase.
- Each normative statement **SHOULD** be addressable by a stable identifier of the form `<Spec-Letter>.<Aspect-Number>.<Statement-Number>`.
...
1. **Kickoff Reflection**
   - *What do I actually believe right now, and how confident?* I believe that while Claude Code has robust documentation, Google Jules and Gemini Deep Research may have sparser public docs. (High confidence).
   - *Strongest piece of evidence against?* Google often releases extensive whitepapers.
   - *Where am I most likely wrong?* Assuming Claude Code is perfectly documented; its community discourse could contradict official docs.
   - *Restart differently?* Prioritize GitHub repos and community reproductions immediately.
   - *Highest-value next action?* Build seed queries and explore docs.
2. **Mid-run Reflection (Jules)**
   - *What do I actually believe right now, and how confident?* Jules is strictly a batch-processing, async VM agent. (High confidence based on ML mastery).
   - *Strongest piece of evidence against?* N/A - all sources point to this VM architecture.
   - *Where am I most likely wrong?* I might be wrong about how Jules handles exploration without explicit file paths.
   - *Restart differently?* Search for more examples of Jules failing.
   - *Highest-value next action?* Search for Claude Code `CLAUDE.md` practices.
3. **Post-Query-Expansion Reflection (Claude)**
   - *What do I actually believe right now, and how confident?* Claude's context window degrades rapidly, making subagents essential. (High confidence).
   - *Strongest piece of evidence against?* Opus 4.7 has a massive context window; theoretically, it shouldn't degrade.
   - *Where am I most likely wrong?* The 60-line limit might be overly strict for simple projects.
   - *Restart differently?* Look for explicit Anthropic guidelines on `CLAUDE.md` length.
   - *Highest-value next action?* Draft Spec-B.
4. **Pre-Synthesis Reflection (Gemini)**
   - *What do I actually believe right now, and how confident?* Gemini DR absolutely requires explicit grounding instructions to perform well. (Medium confidence).
   - *Strongest piece of evidence against?* The prompt's steelman suggests explicit lists suppress diversity.
   - *Where am I most likely wrong?* Assuming it hallucinates without the prompt, it might just be less focused.
   - *Restart differently?* Test if Gemini DR defaults to live search without prompting.
   - *Highest-value next action?* Draft Spec-C.
5. **Post-Synthesis Reflection (Pre-Check)**
   - *What do I actually believe right now, and how confident?* All specs adhere to RFC 2119 and Gherkin. (High confidence).
   - *Strongest piece of evidence against?* I haven't run the bash `grep` check yet.
   - *Where am I most likely wrong?* I might have slipped an all-caps MUST into a rationale block.
   - *Restart differently?* Build an automated linter for RISE-DX output.
   - *Highest-value next action?* Run the assembly and grep check.
6. **Iteration 1 Reflection**
   - *What do I actually believe right now, and how confident?* Jules is an async batch runner. (High confidence).
   - *Where am I most likely wrong?* N/A.
   - *Highest-value next action?* Write Spec-A.
7. **Iteration 2 Reflection**
   - *What do I actually believe right now, and how confident?* Claude is a terminal-based context manager. (High).
   - *Where am I most likely wrong?* N/A.
   - *Highest-value next action?* Write Spec-B.
8. **Iteration 3 Reflection**
   - *What do I actually believe right now, and how confident?* Gemini DR is a long-form report generator. (High).
   - *Where am I most likely wrong?* N/A.
   - *Highest-value next action?* Write Spec-C.
