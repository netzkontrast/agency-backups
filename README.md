...
| [`tools/check-workspace-cleanliness.py`](./tools/check-workspace-cleanliness.py) | Advisory (Task 035 ST-2). Closes [RESEARCH.md R.4.4](./RESEARCH.md) — flags `.py`/`.sh`/`.log` stragglers under `/research/<slug>/workspace/`. A workspace MAY carry a `.cleanignore` listing exempt globs. Set `FM_WORKSPACE_CLEANLINESS_STRICT=1` to gate. |
| [`tools/check-external-result-downstream-task.py`](./tools/check-external-result-downstream-task.py) | Advisory (Task 035 ST-3). Closes [RESEARCH.md R.6.5](./RESEARCH.md) — every `/research/<provider>/<slug>/result.md` MUST be paired with a back-linked Task (`task_affects_paths`, `task_uses_prompts`, `task_spawns_research`, or `task_spawns_prompts`). Set `FM_EXTERNAL_RESULT_STRICT=1` to gate. |
| [`tools/check-trust-audit.py`](./tools/check-trust-audit.py) | Advisory per-workspace GATE (Task 035 ST-4). Implements [RESEARCH.md §5.7](./RESEARCH.md) trust-audit: schema-conformance ≥ 80%, behavioral ≥ 90%, governance ≥ 95% per [`research/agentic-eval-trust-improvement-spec/output/SPEC.md`](./research/agentic-eval-trust-improvement-spec/output/SPEC.md). Diagnostic format `<relpath>::ERROR:TRUST.<code>:<message>`. Module exports `DIAGNOSTIC_SCHEMA` for the AGGREGATOR (Task 039 ST-5). Set `FM_TRUST_AUDIT_STRICT=1` to gate. |
| [`tools/check-readme-frontmatter.py`](./tools/check-readme-frontmatter.py) | ERROR-tier (Task 036 ST-1). Promotes [FOLDERS.md F.5](./FOLDERS.md) from SHOULD to MUST: every operational `readme.md` MUST carry L1 Vault Core frontmatter (`type`, `status`, `slug`, `summary`, `created`, `updated`); the `slug:` MUST contain the parent folder's bare slug. Provider research sub-trees and `/decisions/` are exempt per FOLDERS.md F.1.1 / §8. |
| [`tools/check-audit-graph-consistency.py`](./tools/check-audit-graph-consistency.py) | Advisory (Task 036 ST-2). Closes [FOLDERS.md F.6](./FOLDERS.md) dual-surface drift: warns when a body Markdown link cites a sibling operational folder but the corresponding frontmatter linkage key (`task_uses_prompts`, `prompt_relates_to_task`, etc.) is silent. The reverse asymmetry (frontmatter present, body silent) is NOT flagged — body links are encouraged, not required. Set `FM_AUDIT_GRAPH_STRICT=1` to gate. |
| [`tools/check-clean-working-directory.py`](./tools/check-clean-working-directory.py) | ERROR-tier (Task 037 ST-2). Mechanises [PRE_COMMIT.md PC.1.1](./PRE_COMMIT.md) — flags `.py`/`.sh`/`.log` script-scratchpads anywhere in the working tree outside the FOLDERS.md §8 exempt set (`/tools/`, `/tests/`, `/skills/`, `/templates/`, `/maintenance/`, `/decisions/`, `/Agency-System/`, `/.githooks/`). `session.log` is always allowed. Per-repo carve-outs live in [`tools/.script-allowlist`](./tools/.script-allowlist). Diagnostic format `<relpath>::ERROR:PC.1.1:script-scratchpad`. Wired as step `[2c/6]` of `tools/check-governance.sh`. |
| [`tools/maintenance/staleness-audit.py`](./tools/maintenance/staleness-audit.py) | Advisory (Task 039 ST-3). Implements the deterministic five-signal / three-level / four-leaf decision tree from [`research/spec-staleness-decision-formalization/output/SPEC.md §1`](./research/spec-staleness-decision-formalization/output/SPEC.md) and back-fills [MAINTENANCE.md §3.4](./MAINTENANCE.md). Buckets: `still_accurate` / `drifted` / `completed_by_drift` / `no_longer_desirable`. Reads `MAINT_STALE_DAYS` (default 7); diagnostic / markdown / JSON output formatters. Set `FM_STALENESS_AUDIT_STRICT=1` to gate. |
| [`tools/maintenance/dynamic-readme-partition.py`](./tools/maintenance/dynamic-readme-partition.py) | Advisory (Task 039 ST-4). Enforces the static/dynamic readme partition from [`research/repo-maintenance-protocol-spec/output/SPEC.md §3.1`](./research/repo-maintenance-protocol-spec/output/SPEC.md) and back-fills [MAINTENANCE.md §3.2](./MAINTENANCE.md). Walks `tasks/`, `research/`, `prompts/` for the `<!-- BEGIN/END DYNAMIC -->` markers; emits `M.B.6:missing-marker` / `unbalanced-marker` / `static-below-begin` / `dynamic-above-begin` diagnostics. WARN-tier on missing markers so the existing 170-readme corpus is not retroactively flagged ERROR. |
| [`tools/maintenance/trust-audit.py`](./tools/maintenance/trust-audit.py) | Advisory cross-research AGGREGATOR (Task 039 ST-5; spec-panel C3 partition). Imports `DIAGNOSTIC_SCHEMA` and `audit()` from the per-workspace GATE [`tools/check-trust-audit.py`](./tools/check-trust-audit.py); rolls findings up across all `research/<slug>/` workspaces with `research_phase: complete`; emits FL[1-3]-equivalent `MAINT.TRUST.FRICTION:recommend-task` diagnostics for [MAINTENANCE.md §3.2](./MAINTENANCE.md) friction-aggregation. The `PartitionGuard` test asserts the AGGREGATOR carries no `_score_*` helpers — Spec-J/K/L thresholds remain GATE-owned. Set `FM_TRUST_AUDIT_AGGREGATOR_STRICT=1` to gate. |

A failing linter MUST be fixed (or covered by an unexpired per-rule waiver in `tools/.frontmatter-waivers` per [PRE_COMMIT.md §7.B](./PRE_COMMIT.md)) before the commit can proceed.
