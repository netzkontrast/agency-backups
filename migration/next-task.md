...
4. **Snapshot the to-be-moved set — only Git-controlled root entries from the index, not just HEAD:**
    ...
    - **Untracked-leftovers — RECURSIVE, includes ignored.** Enumerate via TWO queries unioned:
        - `git ls-files --others --exclude-standard` — non-ignored untracked paths.
        - `git ls-files --others --ignored --exclude-standard` — gitignored paths (e.g. local `.env`, `node_modules/`, generated artefacts). Earlier draft used only the first query, which by design omits ignored files — but the spec elsewhere prescribes `git add -f` for them, creating an unreachable branch. Without explicit enumeration here, a local `.env` would silently survive the archive and break the §4 acceptance criterion "Live tree is empty except for migration and .claude".

      Both queries are recursive and surface nested untracked like `tasks/local.tmp`. Filter the unioned list against §3-exempt prefixes (any path under `migration/` or `.claude/` etc.) to get the **untracked-leftovers** set requiring adjudication. `git mv` cannot move untracked entries (it requires tracked sources), so untracked files must be handled explicitly. **Leave-in-place is NOT a permitted option** at this step — it would violate the §4 acceptance criterion. Any untracked file the user wants to keep live MUST first be added to the §3 borderline-keep-live list and re-adjudicated there; only then does it become exempt and skip this step.
