...
| [`research-prompt.md`](./research-prompt.md) | Adversarial state-of-the-art audit prompt for Gemini Deep Research. Covers 12 axes (Ontology, Placement, IDs, Storage, Tooling, Workflows, Governance, Hooks, Migration, Observability, Skill Packaging, Schema Design + the four-layer model itself). Rendered via `skills/research-prompt-optimizer` v3.3.1 in strict interactive mode with 8 methods + B-into-A/C cross-pollination + ReAct/RISEN/Synthesis/TIDD-EC frameworks + 4 constraint blocks + 14 seed queries + 6 orthogonal-lens questions. Full workspace zip (intent + plan view + meta-prompt YAML + rendered prompt) at `/mnt/user-data/outputs/workspace_agency-refactor-soa-audit.zip`. |
