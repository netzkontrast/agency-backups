...
7. **Decide F25 (TodoWrite case sensitivity).** Two-step decision required: (a) confirm the constraint is harness-owned (Claude Code, not repo governance) by checking whether `CLAUDE.md` already mentions tool-schema constraints elsewhere; (b) choose between (i) `landed: <commit>` with a one-line note in [`CLAUDE.md §Using your tools`](../../CLAUDE.md), or (ii) `won't-fix: <reason>` with rationale in this Task's `friction-log.md`. Disposition MUST NOT be pre-judged before this Plan step runs.
