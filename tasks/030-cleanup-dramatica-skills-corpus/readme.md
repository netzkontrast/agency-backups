...
- [`subtasks/`](./subtasks/) — nine `/sc:agent`-dispatchable subtask files; format is PROVISIONAL pending [Task 027](../027-spec-subagent-subtask-prompt-format/).
...
- Spawned prompt: [`/prompts/agency-adr-governance-spec/`](../../prompts/agency-adr-governance-spec/) — the research-proposal prompt that drives Task 027's ratification of the conventions provisionally used here.

## Workflow Assumptions

- Subtask files under [`subtasks/`](./subtasks/) follow the Task 019 layout (briefing + inputs + acceptance criteria + falsification clause + agent-prompt block) because that is the closest precedent in the repo. They are PROVISIONAL: if Task 027's ADR-governance spec invalidates the layout, this task's subtask files re-render under the new contract before Phase A dispatches.
...
- **Verbosity is intentional in `notes.md`.** The user explicitly asked for a "verlose Frustration Log" of the planning session itself so a downstream Task 027 reader can extract pattern requirements. Do not collapse it on the next coherence pass without their consent.
