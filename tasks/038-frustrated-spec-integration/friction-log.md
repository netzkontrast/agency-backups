...
4. **(Added post-review)** TASK.md §4 should require an explicit "deferred-AC → successor-Task filed" checklist item before any agent may flip `task_status: done`. The Task 038 close skipped this step despite the friction-log honestly flagging the deferrals — the reviewer caught what the agent's self-discipline did not. This is a procedure-level fix, not a tooling fix; consider adding it to the FR.B.1 / FR.B.4 Gherkin family.

## §6. PR #87 review-response disposition
