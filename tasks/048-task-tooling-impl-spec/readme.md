...
- **Known spec/implementation drift (surfaced during scaffolding):** TASK.md §7.3 documents the `task_uses_prompts` / `task_spawns_prompts` enforcement scope as `task_status ∈ {done, updated, abandoned}` only, but `tools/fm/validate.py --type-check` (diagnostic code `F.T.1`) currently enforces resolution unconditionally — it rejects pre-declared prompt slugs on `task_status: open` tasks. This Task's frontmatter pre-declares no prompt slugs as a workaround. The drift is a clear candidate for a Task-044-class finding (`F18`?) and SHOULD be filed before this Task's ST-3 SPEC reaches `done`, since ST-3's proposed tooling MUST behave consistently with whichever resolution wins.
