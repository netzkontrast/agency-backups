...
1. The agent MUST produce the artefact required by acceptance criterion: **Surface.** `python3 tools/check-external-result-downstream-task.py [<paths>]`.
2. The agent MUST produce the artefact required by acceptance criterion: **Algorithm.** Detect any `/research/<provider>/<slug>/result.md` lacking a back-linked open Task; emit ERROR-tier diagnostic.
3. The agent MUST produce the artefact required by acceptance criterion: **Diagnostic format.** `<result.md path>::ERROR:R.6.5:no-downstream-task`.
4. The agent MUST produce the artefact required by acceptance criterion: **Tests.** `tests/test_external_result_downstream_task.py` covers: linked Task (pass), missing Task (fail), differently-slugged but back-linked Task (pass via `task_affects_paths`).
5. The agent MUST produce the artefact required by acceptance criterion: **Integration.** `tools/check-governance.sh` step `[1/5]` extension; ERROR-tier (gating).
...
9. The agent MUST commit with a message that names `Task 035 ST-3` in its trailer; the agent MUST NOT push (the maintainer pushes after review).
